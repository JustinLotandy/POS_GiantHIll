<x-app-layout>
    ...
    <div x-data="{ showNotif: {{ count($products->where('stock', '<', 15)) > 0 ? 'true' : 'false' }} }" class="relative">

        {{-- Tombol untuk munculkan popup notif --}}
        <div class="p-4">
            <button @click="showNotif = true" 
                    class="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600">
                🔔 Lihat Notif Stok Rendah
            </button>
        </div>
        ...
        <div x-show="showNotif" 
             x-transition 
             class="fixed top-20 right-6 w-96 bg-white shadow-xl rounded-xl border border-gray-200 z-50">
            <div class="flex justify-between items-center bg-yellow-500 text-white px-4 py-2 rounded-t-xl">
                <span class="font-bold">⚠️ Produk Stok Rendah</span>
                <button @click="showNotif = false" class="hover:text-gray-200">&times;</button>
            </div>
            <div class="p-4 max-h-64 overflow-y-auto">
                @php
                    $lowStock = $products->filter(fn($p) => $p->stock < 15);
                @endphp
                ...
                @if($lowStock->count() > 0)
                    <ul class="list-disc list-inside text-sm text-gray-700 space-y-2">
                        @foreach($lowStock as $p)
                            <li>
                                <b>{{ $p->name }}</b> — stok <span class="text-red-600">{{ $p->stock }}</span>
                            </li>
                        @endforeach
                    </ul>
                @else
                    <div class="text-gray-500 text-sm">Tidak ada produk dengan stok rendah.</div>
                @endif
            </div>
        </div>
