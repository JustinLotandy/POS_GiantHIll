<x-app-layout>
    ...
    {{-- POPUP: tampil kalau ada error stok dari server --}}
    ...
    {{-- POPUP: untuk klik tombol pada kartu stok habis --}}
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-8 mt-5">
                @foreach ($products as $product)
                    <div class="bg-white rounded-xl shadow p-4 flex flex-col items-center border hover:shadow-lg transition relative">
                        ...
                        @if($product->stock <= 0)
                            {{-- Tombol disabled + trigger popup --}}
                            <button type="button"
                                    class="w-full bg-gray-300 text-gray-600 py-1 rounded cursor-not-allowed"
                                    onclick="openEmptyPopup()"
                                    title="Stok habis, tidak bisa ditambahkan">
                                Tambah +
                            </button>
                        @else
    ...
    <script>
        function openEmptyPopup() {
            ...
            wrapper.addEventListener('click', (e) => {
                if (e.target === wrapper) wrapper.remove(); // klik area gelap
            });
            wrapper.querySelectorAll('.btn-popup-close').forEach(btn => {
